...
CF_PRIVATE CFArrayRef __CFStopAllThreads(void) {
    ...
    // 先把从内核那边拿来所有的线程，暂停出自己之外的其他的线程
    // really, should loop doing the stopping until no more threads get added to the list N times in a row
    kern_return_t ret = task_threads(my_task, &thr_list, &thr_cnt); // 给thr_list和thr_cnt赋值
    if (ret == KERN_SUCCESS) {
...
CF_PRIVATE void __CFRestartAllThreads(CFArrayRef threads) {
    for (CFIndex idx = 0; idx < CFArrayGetCount(threads); idx++) {
        ...
        kern_return_t ret = thread_resume(thread); // 核心的方法
...
// 这就是CFRuntimeBase，定义在runtime里面的，大概意思就是定义一个CF的类就都有这个玩意，有点想NSObject里面的isa的意思
...
struct __CFRunLoopMode {
    ...
    // 名字，mode之间的比较就是通过这个名字来比较的，这个玩意真重要
    CFStringRef _name;
    // 暂停的标记
    Boolean _stopped;
    // 填补的字符串？？？
    char _padding[3];
    // 大名鼎鼎的sources0，看到了没，这个是一个集合
    CFMutableSetRef _sources0;
    // 大名鼎鼎的sources1，看到了没，这个是一个集合
    CFMutableSetRef _sources1;
    // 很多的observer，此处是一个数字，因为对完有order返回CFIndex的方法，
    CFMutableArrayRef _observers;
    // 和observer一样，也是数组
    CFMutableArrayRef _timers;
    // source1和mach_port的map字典表，根据port的做key可以寻找到源，可以参考下面的runloop如何添加源
    CFMutableDictionaryRef _portToV1SourceMap;
    
    // 这两个没看明白是什么情况，CFPortSet是一个集合，
    // 创建mode的时候，会把rl的wakeupport、自己_queue的port，mode自己的mktimer的_timerPort也会加入到里面
    __CFPortSet _portSet;
    // 这个玩意就是CFRunLoopActivity的用来记录Observer的状态
    CFIndex _observerMask;
    
#if USE_DISPATCH_SOURCE_FOR_TIMERS
    // timer source 包含一个source、队列、触发日期（如果触发了就会把timer）、
    ...
    // timer触发的实际时间，理想时间加上tolerance的值
...
// 判断runloop的mode是够是一个mode，传参进去的是CFType这个对象，因为这个对象是CF根对象
...
// 释放掉runloop的mode，可以看到里面都是针对结构体的变量进行逐个释放
...
typedef struct _per_run_data {
    uint32_t a;
    uint32_t b;
    uint32_t stopped; // stopped:记录这个runloop是否暂
    uint32_t ignoreWakeUps; // 如果有值的话，可能是忽略wakeup信号，然后直接就wake起来
} _per_run_data;

// 这就是CFRuntimeBase，定义在runtime里面的，大概意思就是定义一个CF的类就都有这个玩意，有点想NSObject里面的isa的意思
typedef struct __CFRuntimeBase {
    uintptr_t _cfisa;
    uint8_t _cfinfo[4]; // 这个数组有4位，通过这个四位的状态来表示这个runloop的状态
    // 睡眠、正在释放、
#if __LP64__
    uint32_t _rc;
#endif
...
struct __CFRunLoop {
    CFRuntimeBase _base; // belong to runtime
    
    pthread_mutex_t _lock;			/* locked for accessing mode list */
    
    // 用于唤醒线程的端口，这个一般用来传递给你mode的portset变量，可能目的就是为了对runloop的wake的事件进行相应
    __CFPort _wakeUpPort;			// used for CFRunLoopWakeUp
    
    Boolean _unused;
    // 应该是之前runloop的数据信息
    //_perRunData{ stopped:记录这个runloop是否暂停}
    ...
    // 几个mode的集合具体是干什么的呢？存储了mode的名字，用来判断是否有某个mode
    CFMutableSetRef _commonModes;
    
    // 注册到commonMode的源
    CFMutableSetRef _commonModeItems;
    
    // 当前runloop的mode，说明每次只能执行一个mode
    CFRunLoopModeRef _currentMode;
    
    // 一个mode的集合，是这个runloop的mode的集合，添加进这个runloop的mode都会放在里面
    CFMutableSetRef _modes;
    
    // 就是添加到runloop里面的block啊，看样子是以链表的形式存在的
    struct _block_item *_blocks_head;
    struct _block_item *_blocks_tail;
    CFTypeRef _counterpart;
};

// CFAllocatorAllocate生成的bit位来决定存储的类型
...
// 给ignoreWakeUps赋值，可能是忽略wakeup信号，然后直接就wake起来
...
// 打印这个runloop的信息，先获取这个cunrent的runloop，然后去调copydescription这个方法
...
// 根据穿进去的runloop、mode名字、是否创建字段，来返回的一个RunLoopMode
static CFRunLoopModeRef __CFRunLoopFindMode(CFRunLoopRef rl, CFStringRef modeName, Boolean create) {
    CHECK_FOR_FORK();
    // 生成一个用来返回的runloop的mode，并实例化这个对象，并且把需要查找的modename赋值给它
    ...
    srlm._name = modeName;
    // 通过srlm来判断rl-->modes里面是否有需要查找的mode
    rlm = (CFRunLoopModeRef)CFSetGetValue(rl->_modes, &srlm);
    
    // 判断查找的结果，如果结果不为空，就锁定这个runloop的mode，然后进行返回
    if (NULL != rlm) {
	__CFRunLoopModeLock(rlm);
	return rlm;
    }
    // 如果调用这个参数的时候，没有指定create为yes，如果没有查找到就直接返回
    if (!create) {
	return NULL;
    }
    // 指定了create为yes，那么就创建一个runloop的mode
    ...
    // 应该注意的GCD里面的dispatch_source_create这个还可以创造其他的资源，为什么要在mode里面去创建一个timer呢？？？
    ...
    // 把传参的runloop的wakeupport座位新生成的mode的protset，这样也是为了能让自己能接收到runloop的wakeup事件
...
// expects rl and rlm locked ： 一般在对runloop的mode来进行操作的时候都会把这个runloop的mode去锁住，那么问题来了，如果对runloop的里面的信息进行操作的时候，是不是应该也把runloop给锁住的呢，这个下面会有答案
// 这个的意思是，判断某个rlm是否为空
static Boolean __CFRunLoopModeIsEmpty(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopModeRef previousMode) {
    ...
    // 1.线程安全就返回false
    // 2.是主线程的runloop
    // 3.传进来的runloop的commonmodes里面包含这个mode
    if (libdispatchQSafe && (CFRunLoopGetMain() == rl) && CFSetContainsValue(rl->_commonModes, rlm->_name)) return false; // represents the libdispatch main queue
    
    // 如果这个mode含有source0的的源数量大于零，就说明这个mode不为空
    if (NULL != rlm->_sources0 && 0 < CFSetGetCount(rlm->_sources0)) return false;
    // 如果这个mode含有source1的的源数量大于零，就说明这个mode不为空
    if (NULL != rlm->_sources1 && 0 < CFSetGetCount(rlm->_sources1)) return false;
    // 如果这个mode含有dispath的timer的的源数量大于零，就说明这个mode不为空
    ...
    while (item) {
        ...
        // 如果链表的mode是string类型，和block的mode一样为string
        if (CFStringGetTypeID() == CFGetTypeID(curr->_mode)) {
            // 如果为mode和rlm的name一样就说明不为空
            // 如果rlm->_name在rl的runloop的commonmodes里面
            doit = CFEqual(curr->_mode, rlm->_name) || (CFEqual(curr->_mode, kCFRunLoopCommonModes) && CFSetContainsValue(rl->_commonModes, rlm->_name));
        } else {
            // 如果block的mode是CFSet
            // block的mode和rlm的那么一样
            // block的mode是kCFRunLoopCommonModes，并且rl的commonModes包含这个name
...
// <--- CF处理runloop类型是否可以用的，当年CF跨平台编译使用的
...
struct __CFRunLoopSource {
    CFRuntimeBase _base;  // runtime定义的类型
    uint32_t _bits; // 判断这个Signal的状态set unset Signaled
    pthread_mutex_t _lock;  // 和runloop一样以及mode一样在设置的都要加锁
    CFIndex _order;			/* immutable */ // 意味这个source不能动，是不是意思不能删除
    
    // NSCountedSet对应的，有便利方法能够直接看某个对象出现的次数
    // 此处应该是能都记录调用这个source的runloop
...
struct __CFRunLoopObserver {
    CFRuntimeBase _base;    // runtime定义的类型，info字段被用来记录正在执行、设置执行、设置非执行，以及下面repeat的三种状态
    
    pthread_mutex_t _lock;  // 和runloop一样以及mode一样在设置的都要加锁
    
    CFRunLoopRef _runLoop;  // oberver观察的runloop
    CFIndex _rlCount;   //
    
    CFOptionFlags _activities;		/* immutable */ // 这个Observer会通过这个字段记录runloop的睡眠，唤醒等状态CFRunLoopActivity
    CFIndex _order;			/* immutable */ // 意味这个source不能动，是不是意思不能删除
...
// 在某个runloop上执行这Observer
static void __CFRunLoopObserverSchedule(CFRunLoopObserverRef rlo, CFRunLoopRef rl, CFRunLoopModeRef rlm) {
    __CFRunLoopObserverLock(rlo);   // 上锁
    if (0 == rlo->_rlCount) {   // 说明这个rlo是初次执行，就把这个rl参数赋值给rlo的runloop属性，也就是说一个rlo只能观察一个runlop
	rlo->_runLoop = rl;
    }
    rlo->_rlCount++;
...
// 取消rlo的观察的行为，和上面CFRunLoopObserverSchedule相反，不过这不会到参数rl和rlm有啥用
...
struct __CFRunLoopTimer {
    CFRuntimeBase _base;    // 问题是为什么这次没有在base里面去保存这些状态
    uint16_t _bits; // 记录timer触发以及Deallocating的状态
    pthread_mutex_t _lock;  // 锁
    
    CFRunLoopRef _runLoop;  // timer依赖的runloop
    
    CFMutableSetRef _rlModes;   // runloop的mode
...
// run loop, we already know that the sources are going to be punted(废弃）
// from it, so invalidation(无效) of sources does not need to remove from a
// deallocating run loop.

// 把某个rlm删除掉和rl有关的souuce
static void __CFRunLoopCleanseSources(const void *value, void *context) {
	...
	if (NULL != rls->_runLoops) {
        // 删除rls-记录的runloops里面的rl这个runloop，
        // TODO: Source里面需要删除rl并没有rlm的事情
...
static void __CFRunLoopDeallocateSources(const void *value, void *context) {
    ...
    if (rlm->_sources0) CFSetGetValues(rlm->_sources0, list);
    if (rlm->_sources1) CFSetGetValues(rlm->_sources1, list + (rlm->_sources0 ? CFSetGetCount(rlm->_sources0) : 0));
   // 先retain一下_sources0和_sources1的对象
    for (idx = 0; idx < cnt; idx++) {
	CFRetain(list[idx]);
    }
    ...
    for (idx = 0; idx < cnt; idx++) {
        ...
        __CFRunLoopSourceLock(rls);
        if (NULL != rls->_runLoops) {
            // 核心的删除动作就是把rl从rls->_runLoops删除了
            CFBagRemoveValue(rls->_runLoops, rl);
        }
        __CFRunLoopSourceUnlock(rls);
        
        // 应该是兼容两种source，因为出现了一种是直接删除，另一种是通过删除这个rlm的port
...
static void __CFRunLoopDeallocateTimers(const void *value, void *context) {
    ...
    void (^deallocateTimers)(CFMutableArrayRef timers) = ^(CFMutableArrayRef timers) {
        ...
        for (idx = 0; idx < cnt; idx++) {
            ...
            // timer这样直接删除记录的模式和删除置顶runloop有些不优雅，但是很直接
...
#pragma mark - CFRunloop 里面的数据结构的实现，和runtime的关系
...
static CFRunLoopRef __CFRunLoopCreate(pthread_t t) {
    ...
    CFRunLoopModeRef rlm;
    // 这里面减掉CFRuntimeBase到底是什么意思
    // 难道CFRuntimeBase不算是runloop里面的构成？？？
    ...
    //TODO:在runloop生成的时候就已经把kCFRunLoopDefaultMode添加到_commonModes里面了！！！
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    // t等于kNilPthreadT的时候，就默认走主线程
    if (pthread_equal(t, kNilPthreadT)) {
	t = pthread_main_thread_np();
    }
    __CFSpinLock(&loopsLock);
    
    // 简历主线程的runloop和pthread_main_thread_np的对应关系
    if (!__CFRunLoops) {
        __CFSpinUnlock(&loopsLock);
        
    // 这个用来记录线程和runloop之间的对应关系
	CFMutableDictionaryRef dict = CFDictionaryCreateMutable(kCFAllocatorSystemDefault, 0, NULL, &kCFTypeDictionaryValueCallBacks);
    // TODO: 创建一个主线程的runloop，这个方法如果多次调用的话，那么就会有多个runloop，估计是不会这样做的
    ...
    // 如果_Runloops已经存在主线程和主runloop的对应，在__CFRunLoops中查找与这个线程对应关系的rl
    CFRunLoopRef loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t));
    __CFSpinUnlock(&loopsLock);
    if (!loop) {    // 没有的话就新建一个loop
	CFRunLoopRef newLoop = __CFRunLoopCreate(t);
        __CFSpinLock(&loopsLock);
	loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t));
	if (!loop) {    // 这个线程和新loop的对应关系放入__CFRunLoops
...
void _CFRunLoopSetCurrent(CFRunLoopRef rl) {
    ...
    if (rl != currentLoop) {
        ...
        __CFSpinLock(&loopsLock);
	if (rl) {
        // TODO:在runloops这个玩意里面几率当前线程指定rl，就完成了设置currentLoop，
        // __CFRunLoops这个是一个字典的类型，里面会记录线程和runloop的关系，那么这个__CFRunLoops到底是啥
...
// TODO:Mode用string标记，几乎所有的操作都是通过mode的name来进行做的
...
// TODO:source、Observer、Timer都属于item
// 把item添加到CommonMode里面去
...
// 这个和上面方法的区别是什么
// 这个是吧ctx里面的item添加到CommonMode里面去
...
CF_EXPORT Boolean _CFRunLoop01(CFRunLoopRef rl, CFStringRef modeName) {
    __CFRunLoopLock(rl);
    // TODO:_commonModes这个能代表所有的mode的名字？？？
...
static Boolean __CFRunLoopDoBlocks(CFRunLoopRef rl, CFRunLoopModeRef rlm) { // Call with rl and rlm locked
    ...
    // block->_mode有两种类型，一种是string一种set
	if (CFStringGetTypeID() == CFGetTypeID(curr->_mode)) {
        // 当前之心的mode和这个block所指定的mode一致的话，或者是kCFRunLoopCommonModes，并且这个mode名字已经被复制到commonModes
	    doit = CFEqual(curr->_mode, curMode) || (CFEqual(curr->_mode, kCFRunLoopCommonModes) && CFSetContainsValue(commonModes, curMode));
        }
    else {
        // 如果这个block的mode的是集合的话，如果当前的mode包含在这个集合里面，以及如果block->mode里面包含有kCFRunLoopCommonModes，都应该判断为真
	    doit = CFSetContainsValue((CFSetRef)curr->_mode, curMode) || (CFSetContainsValue((CFSetRef)curr->_mode, kCFRunLoopCommonModes) && CFSetContainsValue(commonModes, curMode));
	}
	if (!doit) prev = curr;
    // 如果doit为真，那么就应该在当前的这个循环中去设置下一次调用的链表
	if (doit) {
...
static void __CFRunLoopDoObservers(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopActivity activity) {	/* DOES CALLOUT */
    ...
    for (CFIndex idx = 0; idx < cnt; idx++) {
        ...
        // 如果这个Observer没有没执行，有效，那么就把这个Observer给retain一下
    ...
    for (CFIndex idx = 0; idx < obs_cnt; idx++) {
        ...
        __CFRunLoopObserverLock(rlo);
        if (__CFIsValid(rlo)) {
            // 循环触发这个Observer，然后调用回调
            Boolean doInvalidate = !__CFRunLoopObserverRepeats(rlo);
            __CFRunLoopObserverSetFiring(rlo);
            __CFRunLoopObserverUnlock(rlo);
            __CFRUNLOOP_IS_CALLING_OUT_TO_AN_OBSERVER_CALLBACK_FUNCTION__(rlo->_callout, rlo, activity, rlo->_context.info);
            // 如果这个Observer不是重复的，那么就让这个Observer无效
            if (doInvalidate) {
...
// 比较两个Source大小，就是比较他们的order，现在终于知道为什么只有source有order
...
// 从标题看也就只有mode里面需要去收集Sources0，这就是为啥context里面
...
static Boolean __CFRunLoopDoSources0(CFRunLoopRef rl, CFRunLoopModeRef rlm, Boolean stopAfterHandle) {	/* DOES CALLOUT */
    ...
    // TODO:不能都当做数组来处理吗？分开处理的好处？有可能是在底层，对空间的要求非常高，这样的话虽然代码量会增加
    // 但是从运算的角度来说的话，会省掉一些的
...
#pragma mark - 重要的地方开始了

// 定义了__timeout_context、__CFRunLoopTimeoutCancel、__CFRunLoopTimeout
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    // 若果线程安全且当前的runloop是主runloop，并且mode的name属于_commonModes，那就拿到主线程的端口
...
//    };
    ...
    if (seconds <= 0.0) { // instant timeout
        ...
        // 如果传入的时间是一个正常的时间（大于0小于TIMER_INTERVAL_LIMIT）
        // 就用这个时间做一个超时定时器，如果超时了就直接__CFRunLoopTimeoutCancel
    ...
    do {
        ...
        // 执行Observers传入kCFRunLoopBeforeTimers状态
        if (rlm->_observerMask & kCFRunLoopBeforeTimers) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeTimers);
        // 执行Observers传入kCFRunLoopBeforeSourcesv状态
        if (rlm->_observerMask & kCFRunLoopBeforeSources) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeSources);
        
        // 执行置顶rlm下的rl里面的block，是一个链式的循环
        __CFRunLoopDoBlocks(rl, rlm);
        
        // 执行source0里面的回调
        Boolean sourceHandledThisLoop = __CFRunLoopDoSources0(rl, rlm, stopAfterHandle);
        if (sourceHandledThisLoop) {
            // TODO:为什么又跑了一遍置顶mode下rl的block的链表
            __CFRunLoopDoBlocks(rl, rlm);
        }
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    ...
    // 这个mode不能为空啊，也就是mode的源必须有一个值，不然也就没意思啊
    ...
    // 把当前的runloop里的信息以_per_run_data的在存储
    volatile _per_run_data *previousPerRun = __CFRunLoopPushPerRunData(rl);
    CFRunLoopModeRef previousMode = rl->_currentMode;
    rl->_currentMode = currentMode;
    int32_t result = kCFRunLoopRunFinished;
    
    // 根据currentMode的_observerMask来触发Observer不同的状态
	if (currentMode->_observerMask & kCFRunLoopEntry ) __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry);
    // 调用核心的方法
	result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode);
    // 处理runloop的observer的状态
...
// TODO:到底是怎么玩DefaultMode和commonMode的
...
// 通过给runloop的_wakeUpPort发送MACH_SEND_TIMEOUT的信息
...
#pragma mark - 吧block放到runloop里面去执行，就是生成的mode和block组成一个blockitem的链表，然后去把链表去放到rl的tail的后面
